...
double AnalyzerHelpers::calculatePetalAreaModules(const Tracker& tracker, const SimParms& simParms, double crossoverR) {
  ...
  struct PetalAreaVisitor : public ConstGeometryVisitor {
    ...
    void visit(const DetectorModule& m) {
      if (m.side() < 0) return;
      const double petalInterval = tkLayout::TWO_PI / numTriggerProcessorsPhi_; // aka Psi
...
bool AnalyzerHelpers::isModuleInCircleSector(const DetectorModule& module, double sliceMinPhi, double sliceMaxPhi) {
  ...
  double modMinPhi = module.minPhi() >= 0 ? module.minPhi() : module.minPhi() + tkLayout::TWO_PI;
  double modMaxPhi = module.maxPhi() >= 0 ? module.maxPhi() : module.maxPhi() + tkLayout::TWO_PI;


  if (modMinPhi > modMaxPhi && sliceMaxPhi > tkLayout::TWO_PI) modMaxPhi += tkLayout::TWO_PI;      // this solves the issue with modules across the 2 PI line
  else if (modMinPhi > modMaxPhi && sliceMaxPhi < tkLayout::TWO_PI) modMinPhi -= tkLayout::TWO_PI; // 

  bool inSectorSlice = ((sliceMinPhi < modMaxPhi && modMinPhi < sliceMaxPhi) ||
                        (sliceMinPhi < modMaxPhi+tkLayout::TWO_PI && modMinPhi+tkLayout::TWO_PI < sliceMaxPhi) || // this catches the modules that are at a small angle but must be caught by a sweep crossing the 2 PI line
                        (sliceMinPhi < modMaxPhi-tkLayout::TWO_PI && modMinPhi-tkLayout::TWO_PI < sliceMaxPhi));
...
bool AnalyzerHelpers::isModuleInPhiSector(const SimParms& simParms, const DetectorModule& module, double crossoverR, int phiSector) {
  ...
  double phiSlice = tkLayout::TWO_PI / simParms.numTriggerTowersPhi();  // aka Psi
